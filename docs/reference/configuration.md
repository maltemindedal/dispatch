...
Engine settings left unset use the engine's own defaults (`QueueConfig.Builder` for the
...
| `worker-id` | string | `""` (generate) | This instance's identity in `locked_by`. Blank means "generate a unique one at startup" (hostname-ish plus random suffix), which is useful in containers. If set, it must be unique per process. Two instances sharing an id can release each other's leases. |
...
Two tuning notes apply to that profile:

- Hikari's `maximum-pool-size` is `24`, above the default `concurrency` of 16,
