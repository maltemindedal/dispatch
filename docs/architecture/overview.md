...
components inside the engine, and separate roles for platform and virtual threads.
...
| `dispatch-postgres` | `dispatch-core` | `JdbcJobRows`, the same engine backed by PostgreSQL or H2 and using `SELECT ... FOR UPDATE SKIP LOCKED`. Still no Spring. |
| `dispatch-api` | both | Spring Boot: REST controllers, configuration properties, profile wiring. |

Dependencies only point inward. `dispatch-core` cannot see Spring or JDBC. The build enforces that
...
- **`JobQueue`** is the engine's entry point. It wires together a store, a handler registry, a worker pool, and a
  maintenance sweeper. Submitting inserts a row and wakes the dispatcher, so
  locally produced work skips the poll interval.
- **`WorkerPool`** loops in a single dispatcher thread. It reserves capacity, claims up to a batch of
  jobs, and hands each to its own virtual thread. Results are written back conditionally on still
...
- **`QueueMaintenance`** sweeps on a timer with two idempotent jobs. It promotes `SCHEDULED`/`FAILED`
  rows whose time has come, and returns `RUNNING` rows with expired leases to `PENDING`. Every
...
- **`JobRows`** is the persistence seam underneath, and it decides nothing. It holds these rows
  exclusively and writes them back. `InMemoryJobRows` uses a lock over a map, `JdbcJobRows` uses
  a transaction and `FOR UPDATE`. Both are held to the same contract test suite
  (`JobStoreContract`) through `JobStore`, so the shared tests verify that the adapters can be swapped.
...
Each handler runs on a **virtual thread**. These threads are short-lived and mostly blocked on
...
A claim-capacity gate sized to `concurrency`, with one permit per job in flight reserved before
...
- [Reliability mechanics](reliability.md). Claiming, leases, delivery guarantees, and shutdown.
- [Known limitations](limitations.md). What the system deliberately does not include.
